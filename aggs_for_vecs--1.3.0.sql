...
CREATE OR REPLACE FUNCTION
vec_to_stats_transfn(internal, anyarray)
RETURNS internal
AS 'aggs_for_vecs', 'vec_to_stats_transfn'
LANGUAGE c;

CREATE OR REPLACE FUNCTION
vec_to_stats_finalfn(internal, anyarray)
RETURNS integer[]
AS 'aggs_for_vecs', 'vec_to_stats_finalfn'
LANGUAGE c;

CREATE AGGREGATE vec_to_stats(integer[]) (
  sfunc = vec_to_stats_transfn,
  stype = internal,
  finalfunc = vec_to_stats_finalfn,
  finalfunc_extra
);
